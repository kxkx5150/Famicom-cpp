...
class FirstOrderFilter {
  ...
  public:
    ...
    FirstOrderFilter(uint32_t f, uint32_t sample_rate)
        : RC{1.0 / (2 * 3.1415928535 * double(f))}, dt{1.0 / double(sample_rate)}
    {
    }
};
class LoPassFilter final : public FirstOrderFilter {
  ...
  public:
    ...
    LoPassFilter(uint32_t f, uint32_t sample_rate)
        : FirstOrderFilter(f, sample_rate), a{this->dt / (this->RC + this->dt)}, prev{0, 0}
    {
    }
...
class HiPassFilter final : public FirstOrderFilter {
  ...
  public:
    ...
    HiPassFilter(uint32_t f, uint32_t sample_rate)
        : FirstOrderFilter(f, sample_rate), a{this->RC / (this->RC + this->dt)}, prev{0, 0}
    {
    }
